-- INVOTATION V22 — Accountless QR Device Pairing
-- 1) Enable Anonymous Sign-Ins in Supabase Authentication settings.
-- 2) Run this script ONCE in SQL Editor.
...
using (
  exists (
    select 1
    from public.invotation_space_members m
    where m.space_id = invotation_spaces.id
...
using (
  exists (
    select 1
    from public.invotation_space_members mine
    where mine.space_id = invotation_space_members.space_id
